...
function MemberList () {
  ...
  useEffect(() => {
    const userListData = async () => {
      const response = await axios.get('http://3.34.124.123:8080/user/userManage');
  ...
  const handleDelete = async(userId) => {
    ...
    if (confirmed) {
      await axios.delete(`http://3.34.124.123:8080/user/signOut/${userId}`);
  ...
  const handleApproval = async(userId) => {
    ...
    if (confirmed) {
      await axios.put(`http://3.34.124.123:8080/user/signAccept/${userId}`);
      setMemberListData(memberListData.filter(item => item.userId !== userId));
      alert("승인되었습니다.");
    } else {
