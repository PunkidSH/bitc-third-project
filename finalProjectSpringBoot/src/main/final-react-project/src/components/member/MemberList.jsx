...
function MemberList () {
  ...
  useEffect(() => {
    const userListData = async () => {
      const response = await axios.get('http://15.164.61.252:8080/api/user/userManage');
  ...
  const handleDelete = async(userId) => {
    ...
    if (confirmed) {
      await axios.delete(`http://15.164.61.252:8080/api/user/signOut/${userId}`);
  ...
  const handleApproval = async(userId) => {
    ...
    if (confirmed) {
      await axios.put(`http://15.164.61.252:8080/api/user/signAccept/${userId}`);
      setMemberListData(memberListData.filter(item => item.userId !== userId));
      alert("승인되었습니다.");
    } else {
